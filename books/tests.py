...
class BooksTestCase(TestCase):
    ...
    def test_book_list(self):
        ...
        response = self.client.get(reverse("books:list") + "?page=2&page=2")
    ...
    def test_title_detail(self):
        ...
        response = self.client.get(reverse("books:datail", kwargs={"id": book.id}))
    ...
    def test_search_books(self):
        ...
        book2 = Book.objects.create(title="news", description="description2", isbn="1235456")
        book3 = Book.objects.create(title="history", description="description3", isbn="000020")

        resource = self.client.get(reverse("books:list") + "?q=sport")
        ...
        resource = self.client.get(reverse("books:list") + "?q=news")
        self.assertContains(resource, book2.title)
        self.assertNotContains(resource, book1.title)
        self.assertNotContains(resource, book3.title)

        resource = self.client.get(reverse("books:list") + "?q=history")
